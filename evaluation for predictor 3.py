...
Reference:
Guth, S. & Sapsis, T.P. Machine Learning Predictors of Extreme Events Occurring in Complex Dynamical Systems.
...
def align_arrays(a, b):
    """
    Align two arrays by flattening and truncating to the minimum length.
    Returns arrays with the same length.
    """
    a = np.asarray(a).flatten()
    b = np.asarray(b).flatten()
    minlen = min(len(a), len(b))
    if len(a) != len(b):
        print(f"Warning: Arrays are different lengths ({len(a)} vs {len(b)}). Truncating to {minlen}.")
...
def main(actual_path, predicted_path, percentiles=None, output_dir=None):
    ...
    try:
        ...
        Actual_raw = np.load(actual_path)
        predicted_raw = np.load(predicted_path)

        Actual, predicted = align_arrays(Actual_raw, predicted_raw)
        ...
        if output_dir:
            ...
            print(f"Best PR curve saved to {os.path.join(output_dir, 'pr_curve_best.csv')}")
...
if __name__ == "__main__":
    actual_path = "Actual.npy"
    predicted_path = "Predicted_tau_30 (2).npy"
    percentiles = np.linspace(80, 99.5, 100)
